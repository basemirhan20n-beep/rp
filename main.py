...
from futbol import (
    ...
    futbol_callback, set_group_chat_id, GROUP_CHAT_ID
)
...
def seviye_hesapla(xp):
    return max([lvl for lvl, req in SEVIYE_ESLEME.items() if xp >= req], default=1)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Sayın {user.first_name}, günlük görevinizi yaparak XP ve güven kazanın.",
        parse_mode="Markdown",
...
async def _profil_goster(update, veri, isim):
    ...
    durum = (
        "🟢 Güvenli" if guven >= 75
        else "🟡 Dikkatli" if guven >= 50
        else "🟠 Riskli" if guven >= 30
        else "🔴 Kritik"
    )
    mesaj = (
        f"👤 *{isim} — Profil*\n{'-'*28}\n"
...
async def makam_komutu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rol:
        ...
        durum = (
            "🟢 Güvenli Koltuk" if guven >= 75
            else "🟡 Dikkat Gerektiren Koltuk" if guven >= 50
            else "🔴 Riskli Koltuk" if guven >= 30
            else "⛔ Kritik — Görevden Alma Süreci"
        )
...
async def gorev_yap_komutu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not veri or not veri["role"]:
        ...
        await reply("⚠️ Makam atanmamış.")
        return
    bugun = date.today().isoformat()
    if veri["last_task"] == bugun:
        reply = update.message.reply_text if update.message else update.callback_query.edit_message_text
        await reply("✅ Bugünkü görevinizi zaten tamamladınız.")
        return
...
async def gorev_tamamla(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db.kullanici_guncelle(
        user.id, xp=yeni_xp, level=yeni_level, guven=yeni_guven,
        streak=streak, last_task=date.today().isoformat()
    )
    mesaj_metni = random.choice(GOREV_MESAJLARI[rol])
    seviye_mesaji = (
        f"\n\n🎉 *SEVİYE ATLADI!* → Seviye {yeni_level}"
        if yeni_level > seviye_hesapla(veri["xp"]) else ""
    )
    mesaj = (
        f"✅ *Görev Tamamlandı*\n─────────────────────\n{mesaj_metni}\n\n"
...
async def rol_ver(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) < 2:
        return await update.message.reply_text("⚠️ Kullanım: `/rol_ver @kullanıcı Makam Adı`", parse_mode="Markdown")
    ...
    if not hedef:
        return await update.message.reply_text(f"⚠️ `{hedef_username}` bulunamadı.")
    db.rol_ata(hedef["user_id"], makam)
    await update.message.reply_text(
        f"✅ *{hedef_username}* kullanıcısına *{makam}* görevi verildi.", parse_mode="Markdown"
    )
...
async def rol_al(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await is_admin(update, context):
        return await update.message.reply_text("⛔ Yetkisiz.")
    if not context.args:
        return await update.message.reply_text("⚠️ Kullanım: `/rol_al @kullanıcı`", parse_mode="Markdown")
...
async def duyuru(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await is_admin(update, context):
        return await update.message.reply_text("⛔ Yetkisiz.")
    if not context.args:
        return await update.message.reply_text("⚠️ Kullanım: `/duyuru Mesaj`", parse_mode="Markdown")
...
async def puan_ver(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) < 2:
        return await update.message.reply_text("⚠️ Kullanım: `/puan_ver @kullanıcı miktar`", parse_mode="Markdown")
...
async def guven_ver(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) < 2:
        return await update.message.reply_text("⚠️ Kullanım: `/guven_ver @kullanıcı miktar`", parse_mode="Markdown")
    hedef_username, miktar = args[0].lstrip("@"), int(args[1])
    hedef = db.kullanici_username_ile_getir(hedef_username)
    if not hedef:
        return await update.message.reply_text(f"⚠️ `{hedef_username}` bulunamadı.")
    ...
    await update.message.reply_text(
        f"✅ *{hedef_username}* güveni *{miktar}* değişti. Yeni güven: {yeni_guven}",
        parse_mode="Markdown",
    )
...
async def sezon_sifirla_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await is_admin(update, context):
        return await update.message.reply_text("⛔ Yetkisiz.")
    await cmd_sezon_sifirla(update, context)
...
async def buton_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "profil":
        ...
        klavye = [
            ...
            [InlineKeyboardButton("🛒 Transfer Piyasası", callback_data="piyasa_0"),
            ...
            [InlineKeyboardButton("🎡 Çark", callback_data="cark"),
             InlineKeyboardButton("👑 Loncalar", callback_data="loncalar")],
            [InlineKeyboardButton("🔙 Ana Menü", callback_data="ana_menu")],
        ]
        await query.edit_message_text(
            "⚽ *Cumhuriyet Süper Ligi*", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(klavye)
        )
...
def main():
    ...
    app.add_handler(CommandHandler("sezon_sifirla", sezon_sifirla_admin))
