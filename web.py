...
def lig_tablosu(lig_id):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT isim, puan, galibiyet, beraberlik, maglubiyet, atilan_gol, yenilen_gol "
        "FROM takimlar WHERE lig_id=? ORDER BY puan DESC",
        (lig_id,),
    )
    rows = cur.fetchall()
    conn.close()
    return jsonify([
        {"isim": r[0], "puan": r[1], "galibiyet": r[2], "beraberlik": r[3],
         "maglubiyet": r[4], "atilan_gol": r[5], "yenilen_gol": r[6]}
        for r in rows
    ])
...
def gol_krallari():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT o.isim, o.gol, t.isim AS takim FROM oyuncular o "
        "JOIN takimlar t ON o.takim_id=t.takim_id ORDER BY o.gol DESC LIMIT 10"
    )
    rows = cur.fetchall()
    conn.close()
