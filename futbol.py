"""
Cumhuriyet Süper Ligi — Futbol Modülü (güncel)
"""
...
def _piyasa_markup(sayfa, toplam, sayfa_boyut=8):
    satirlar, nav = [], []
    if sayfa > 0:
        nav.append(InlineKeyboardButton("◀️ Önceki", callback_data=f"piyasa_{sayfa-1}"))
    ...
    if (sayfa + 1) * sayfa_boyut < toplam:
        nav.append(InlineKeyboardButton("Sonraki ▶️", callback_data=f"piyasa_{sayfa+1}"))
    if nav:
        satirlar.append(nav)
...
async def cmd_takim_kur(update, context):
    ...
    if not context.args:
        return await update.message.reply_text("⚽ Kullanım: `/takim_kur Takım Adı`", parse_mode="Markdown")
    isim = " ".join(context.args)
    basari, mesaj = fdb.takim_kur(user.id, isim)
    if not basari:
        return await update.message.reply_text(f"❌ {mesaj}")
    ...
    await update.message.reply_text(
        f"✅ *{isim}* takımı kuruldu!\n💰 Başlangıç bütçen: *100.000₺*\n"
        f"👥 Şu an ligde *{takim_sayisi}/{LIG_TAKIM_LIMITI}* takım var.\n"
        f"📋 Oyuncu almak için: `/piyasa`{fikstur_mesaj}",
        parse_mode="Markdown",
    )
...
async def cmd_takim(update, context):
    ...
    if not takim:
        return await _gonder(update, "❌ Takımın yok. `/takim_kur <Ad>` ile kur.", parse_mode="Markdown")
    ...
    metin = (
        f"🏟️ *{takim['isim']}*\n{'─'*30}\n"
        f"💰 Bütçe: *{para:,}₺*\n⚽ Ortalama Güç: *{guc}*\n"
        f"📊 {takim['puan']} puan — {takim['galibiyet']}G {takim['beraberlik']}B {takim['maglubiyet']}M — "
        f"{takim['atilan_gol']}:{takim['yenilen_gol']}\n"
        f"👥 Kadro: *{len(oyuncular)}/23* oyuncu\n{'─'*30}\n"
    )
    if oyuncular:
...
async def _piyasa_goster(update, sayfa):
    if not fdb.transfer_acik_mi():
        return await _gonder(update, "❌ Transfer dönemi kapalı. Belirli günlerde tekrar dene.")
    oyuncular, toplam = fdb.piyasa(sayfa)
    if not oyuncular:
        return await _gonder(update, "🛒 Piyasada şu an oyuncu yok.")
    metin = f"🛒 *Transfer Piyasası* ({toplam} oyuncu)\n{'─'*32}\n"
    for o in oyuncular:
        takim_info = "🔓 Serbest" if not o["takim_id"] else "🏟️ Transfer"
        metin += (
            f"{POZ_EMOJI.get(o['pozisyon'], '⚽')} `[{o['oyuncu_id']}]` *{o['isim']}* — {o['pozisyon']}\n"
            f"   Güç: {o['guc']} | 💰 {o['satis_fiyati']:,}₺ | {takim_info}\n"
        )
...
async def cmd_satin_al(update, context):
    ...
    if not fdb.transfer_acik_mi():
        return await update.message.reply_text("❌ Transfer dönemi kapalı.")
    if not context.args or not context.args[0].isdigit():
        return await update.message.reply_text("⚠️ Kullanım: `/satin_al <oyuncu_id>`", parse_mode="Markdown")
...
async def cmd_sat(update, context):
    ...
    takim = fdb.takim_user(user.id)
    if not takim:
        return await update.message.reply_text("❌ Takımın yok.")
    if len(context.args) < 2:
        return await update.message.reply_text(
            "⚠️ Kullanım: `/sat <oyuncu_id> <fiyat>`\nSatışı iptal: `/sat_iptal <oyuncu_id>`",
            parse_mode="Markdown",
        )
    try:
        oyuncu_id, fiyat = int(context.args[0]), int(context.args[1])
    except Exception:
        return await update.message.reply_text("❌ ID ve fiyat sayısal olmalı.")
...
async def cmd_sat_iptal(update, context):
    ...
    if not takim:
        return await update.message.reply_text("❌ Takımın yok.")
    if not context.args or not context.args[0].isdigit():
        return await update.message.reply_text("⚠️ Kullanım: `/sat_iptal <oyuncu_id>`", parse_mode="Markdown")
    oyuncu_id = int(context.args[0])
...
async def cmd_antrenman(update, context):
    ...
    takim = fdb.takim_user(user.id)
    if not takim:
        return await _gonder(update, "❌ Takımın yok.", parse_mode="Markdown")
    basari, sonuc = fdb.antrenman_yap(takim["takim_id"])
    if not basari:
        return await _gonder(update, f"⚠️ {sonuc}")
    metin = f"🏋️ *Antrenman Tamamlandı — {takim['isim']}*\n{'─'*30}\n"
    for o in sonuc:
        metin += f"{POZ_EMOJI.get(o['pozisyon'], '⚽')} *{o['isim']}*\n   Güç: +{o['artis']} → **{o['yeni_guc']}**\n"
...
async def cmd_mac(update, context):
    ...
    if not fdb.fikstur_var_mi(takim["lig_id"]):
        ...
        return await _gonder(
            update,
            f"⏳ Fikstür henüz oluşturulmadı. Lig başlaması için *{kalan}* takım daha gerekiyor.",
            parse_mode="Markdown",
        )
    yeterli, neden = fdb.yeterli_kadro_mu(takim["takim_id"])
    if not yeterli:
        return await _gonder(update, f"❌ {neden}")
    mac = fdb.sonraki_mac(takim["takim_id"])
    if not mac:
        return await _gonder(update, "🎉 Sezon bitti! Tüm maçlarını oynadın.")
    ev_takim = fdb.takim_id(mac["ev_takim_id"])
    dep_takim = fdb.takim_id(mac["dep_takim_id"])
    metin = (
        f"⚽ *Cumhuriyet Süper Ligi — {mac['hafta']}. Hafta*\n{'─'*32}\n"
        f"🏠 {ev_takim['isim']} (Güç: {round(fdb.takim_gucu(mac['ev_takim_id']),1)})\n"
        f"✈️ {dep_takim['isim']} (Güç: {round(fdb.takim_gucu(mac['dep_takim_id']),1)})\n\n"
...
async def cmd_lig(update, context):
    ...
    if not takimlar:
        return await _gonder(update, f"{LIG_ADI}\n\nHenüz takım yok.")
    metin = (
        f"{LIG_ADI}\n{'═'*32}\n"
        f"{'#':<3} {'Takım':<18} {'O':>2} {'G':>2} {'B':>2} {'M':>2} {'AG':>3} {'YG':>3} {'P':>3}\n"
        f"{'─'*32}\n"
    )
    rozetler = ["🥇", "🥈", "🥉"]
    for i, t in enumerate(takimlar):
        ...
        metin += (
            f"{rozet} `{t['isim'][:14]:<14} {t['mac_sayisi']:>2} {t['galibiyet']:>2} "
            f"{t['beraberlik']:>2} {t['maglubiyet']:>2} {t['atilan_gol']:>3} "
            f"{t['yenilen_gol']:>3} {t['puan']:>3}`\n"
        )
    if not fdb.fikstur_var_mi(1):
        kalan = LIG_TAKIM_LIMITI - len(takimlar)
        metin += f"\n⏳ Lig başlaması için *{kalan}* takım daha gerekli."
    markup = InlineKeyboardMarkup([[
        InlineKeyboardButton("🔄 Yenile", callback_data="lig_tablosu"),
...
async def cmd_son_maclar(update, context):
    ...
    takim = fdb.takim_user(user.id)
    if not takim:
        return await _gonder(update, "❌ Takımın yok.")
    maclar = fdb.son_maclar(takim["takim_id"])
    if not maclar:
        return await _gonder(update, "📋 Henüz hiç maç oynamadın.")
...
async def _fikstur_goster(update, hafta):
    ...
    if not maclar:
        return await _gonder(update, f"📅 {hafta}. haftada maç yok.")
    metin = f"📅 *{LIG_ADI} — {hafta}. Hafta*\n{'─'*32}\n"
    for m in maclar:
        if m["oynanmis"]:
            skor, durum = f"*{m['ev_gol']}–{m['dep_gol']}*", "✅"
        else:
            skor, durum = "vs", "⏳"
        metin += f"{durum} {m['ev_isim']} {skor} {m['dep_isim']}\n"
    markup = InlineKeyboardMarkup([[
        InlineKeyboardButton("◀️", callback_data=f"fikstur_{max(1, hafta-1)}"),
        InlineKeyboardButton(f"Hafta {hafta}", callback_data="noop"),
        InlineKeyboardButton("▶️", callback_data=f"fikstur_{hafta+1}"),
    ]])
    await _gonder(update, metin, markup=markup)
...
async def cmd_cuzdan(update, context):
    ...
    await _gonder(
        ...
        f"_Maç kazanınca 5.000₺, beraberlikte 2.500₺, kayıpta 1.000₺ kazanırsın._",
        parse_mode="Markdown",
    )
...
async def cmd_taktik(update, context):
    ...
    if not takim:
        return await update.message.reply_text("❌ Takımın yok.")
    if not context.args or context.args[0] not in fdb.TAKTIKLER:
        return await update.message.reply_text(f"⚠️ Kullanım: /taktik {'|'.join(fdb.TAKTIKLER)}")
...
async def cmd_lonca_kur(update, context):
    if not context.args:
        return await update.message.reply_text("⚠️ Kullanım: /lonca_kur LoncaAdı")
    isim = " ".join(context.args)
...
async def cmd_lonca_katil(update, context):
    if not context.args:
        return await update.message.reply_text("⚠️ Kullanım: /lonca_katil LoncaID")
    try:
        lid = int(context.args[0])
    except Exception:
        return await update.message.reply_text("❌ Lonca ID sayı olmalı.")
...
async def cmd_kupa(update, context):
    ...
    if not mac:
        return await update.message.reply_text("🏆 Şu an aktif kupa maçın yok veya kupa oluşturulmamış.")
    await _gonder(
        update,
        f"🏆 *Cumhuriyet Kupası* - {mac['tur']}. Tur\n{mac['ev_isim']} vs {mac['dep_isim']}\n"
        f"Maçı başlatmak için /mac komutunu kullan.",
    )


async def cmd_sezon_sifirla(update, context):
    # Admin check is handled in main.py before calling this
    fdb.sezon_sifirla()
...
async def cmd_oyuncu_istatistik(update, context):
    if not context.args or not context.args[0].isdigit():
        return await update.message.reply_text("⚠️ Kullanım: /oyuncu_istatistik <oyuncu_id>")
    istatistik = fdb.oyuncu_istatistikleri(int(context.args[0]))
    if not istatistik:
        return await update.message.reply_text("Oyuncu bulunamadı.")
    await update.message.reply_text(
        f"📊 *{istatistik['isim']}* istatistikleri\n"
        f"Gol: {istatistik['gol']}\nAsist: {istatistik['asist']}\n"
        f"Sarı Kart: {istatistik['sari_kart']}\nKırmızı Kart: {istatistik['kirmizi_kart']}\n"
        f"Sakatlık (kalan maç): {istatistik['sakatlik_mac']}",
        parse_mode="Markdown",
    )
...
async def cmd_altyapi(update, context):
    ...
    if not oyuncu:
        return await update.message.reply_text(
            "❌ Altyapıdan oyuncu çıkarılamadı. (Kadro dolu veya günlük limit?)"
        )
    await update.message.reply_text(
        f"🌟 Altyapıdan yeni oyuncu çıktı!\n"
        f"{POZ_EMOJI.get(oyuncu['pozisyon'], '⚽')} *{oyuncu['isim']}* — {oyuncu['pozisyon']} | Güç: {oyuncu['guc']}\n"
        f"Kadrona eklendi.",
        parse_mode="Markdown",
    )
...
async def futbol_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if data.startswith("mac_bas_"):
        ...
        if eg > dg:
            sonuc_emoji = "🟢 Ev sahibi kazandı!"
        elif dg > eg:
            sonuc_emoji = "🔵 Deplasman kazandı!"
        else:
            sonuc_emoji = "🟡 Berabere!"
        metin = (
        ...
        odul = (
            5000 if (eg > dg and takim["isim"] == ev) or (dg > eg and takim["isim"] == dep)
            else (1000 if (eg > dg and takim["isim"] == dep) or (dg > eg and takim["isim"] == ev) else 2500)
        )
        metin += f"\n💰 Kazanılan ödül: *+{odul:,}₺*"
        markup = InlineKeyboardMarkup([[
            InlineKeyboardButton("📊 Lig Tablosu", callback_data="lig_tablosu"),
    ...
    if data == "fikstur_goster":
        await query.answer()
        await _fikstur_goster(update, fdb.mevcut_hafta())
        return True

    if data.startswith("fikstur_"):
        await query.answer()
        await _fikstur_goster(update, int(data.split("_")[1]))
        return True
    ...
    if data == "cark":
        await query.answer()
        await cmd_cark(update, context)
        return True

    if data == "loncalar":
        loncalar = fdb.lonca_listesi()
        metin = (
            "👑 *Loncalar*\n" + "\n".join(f"{l['lonca_id']}: {l['isim']} (Puan:{l['puan']})" for l in loncalar)
            if loncalar
            else "Hiç lonca yok."
        )
